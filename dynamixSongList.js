...
(async () => {
    ...
    await page.goto('https://dynamixc4cat.fandom.com/wiki/Songs_by_Artist');
    console.log('\n\n\n****************************************************************************************************')
    const tableRows = await page.$$('table.wikitable tbody tr');

    let songRow = [];

    for (const row of tableRows) {
        const [numberElement, titleElement, artistElement] = await row.$$('td:nth-child(1), td:nth-child(2), td:nth-child(3)');
        const number = await numberElement.evaluate((node) => node.textContent);
        const title = await titleElement.$eval('a', (a) => a.textContent);
        const artist = await artistElement.evaluate((node) => node.textContent);
        // console.log(`Number: ${number.trim()}, Title: ${title.trim()}, Artist: ${artist.trim()}`);
        songRow =
            [
                number.trim(),
                title.trim(),
